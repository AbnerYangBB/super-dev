...
2. 首次执行“影响面分析与调用链定位”前，先检查 `serena` 是否可用。
3. 未激活时，按顺序调用：
   `serena.activate_project` -> `serena.check_onboarding_performed` -> `serena.initial_instructions`。

## 3) 分析
1. 影响面分析与调用链定位优先使用 `serena`（文件、符号、引用）。
2. 不确定 API 或框架用法时，使用 `context7` 校验，优先官方文档。
3. 复杂问题（多约束、方案冲突、边界不清）优先使用相关 skills，必要时使用 `sequential-thinking` 进行拆解。

## 4) iOS 构建与运行
1. 此项目优先使用 `xcode-builder` skills 构建；简单任务可只做静态验证，复杂功能完成后必须构建验证。
2. 运行 App 优先使用 `ios-device-runner` skills，默认真机运行。

## 5) 本地化与提交
1. 提交代码时，若涉及用户可见文案变更，必须使用 `sync-add-ios-loc` skills。
